...
class appServer extends database{
     ...
     * @return array
     */
    public function fetchResources($counter){
        $data = array() ;
        $qry = $this->query("SELECT * FROM {$this->resources_tbl} LIMIT $counter,".$this->limit) ;
     ...
     * @return array
     */
    public function fetchDownloads($counter){
        $data = array() ;
        $qry = $this->query("SELECT file,title FROM {$this->downloads_tbl} LIMIT $counter,".$this->limit) ;
        while( $r = $qry->fetch_assoc() ){
            if($r){
     ...
     */
    public function fetchTestimonies($counter){
        $accounts = new accounts();
        $data = array() ;
        $i = 0 ;
        $qry = $this->query("SELECT testimony_id,uid,message,time FROM {$this->testimony_tbl} LIMIT $counter,".$this->limit) ;
        while( $r = $qry->fetch_assoc() ){
            if($r){
