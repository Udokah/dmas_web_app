...
$app->get('/testimony', 'authenticate' , function() use ($app){
    ...
    // check for required params
    verifyRequiredParams(array('counter'));
    $response = array();

    $account = new accounts() ;
    $appServer = new appServer() ;

    $counter = $account->cleanUP( $app->request->get('counter') );
    $result = $appServer->fetchTestimonies( $counter );
...
$app->get('/downloads', 'authenticate' , function() use ($app){
    ...
    $appServer = new appServer() ;
    $downloadsList = $appServer->fetchDownloads($counter);
...
$app->get('/resources', 'authenticate' , function() use ($app){
    ...
    $appServer = new appServer() ;
    $resourceList = $appServer->fetchResources($counter);
